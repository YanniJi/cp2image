...
class Trainer:
	...
	def __init__(self,
				name,
				file2record_loss = None,
				testfile2record_loss = None, 
		...
		self._file2record_loss = open(file2record_loss, 'w')
		self._testfile2record_loss = open(testfile2record_loss, 'w')
		self._rand_img = np.random.normal(size=(128,68,68,3)).astype(np.float32)
		##--------
		...
		with self.graph.as_default():
				...
				self.session = tf.Session(graph=self.graph, config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=True), **gpu_config_kwargs) #-- Creation of the tensorflow session
...
# 						print(">> SESSION RESTORED step[{}].".format(self._step))
	...
	*** TRAINING METHODS
	...
	def train(self,
			...
			tensor_images_disc,
			features): #-- Batch of real images for discrimination
...
# 		print(">> MODEL UPDATE (VAEPlus) {}".format(self._step))
		...
		with self.graph.as_default():
				...
				#-- No input embedding
				feed_dict[self.embedding_ph] = features.astype(np.float32)
...
# 				## test encoder weights not change     
# 				feature_mean, feature_var = self.session.run([self.embedding_means, 
# 				                                              self.embedding_logvars],
# 				                                              feed_dict = feed_dict)                
# 				print(feature_mean.mean(), feature_var.mean(), feature_var.mean(), feature_var.std(), feature_mean.shape, feature_var.shape)
# 				print(feature_mean[10, :, :, 10:20])
# 				print(feature_var[10, :, :, 10:20]) 
				##
				##-- DISPLAY INFO
# 				print(">> RGB RECONSTRUCTION LOSS: ", reconstructionLoss_RGB)
# 				print(">> GAN RECONSTRUCTION LOSS: ", reconstructionLosses_GAN[-1])
				self._file2record_loss.write('{}: TRAIN RGB RECON LOSS: {:.4f}, GAN RECONS LOSS: {:.4f}, GAN LOSS:{:.4f}\n'.format(self._step, reconstructionLoss_RGB, reconstructionLosses_GAN[-1], _GAN_loss ))
				self._file2record_loss.flush()
        
	def validate(self,
			tensor_images,      #-- Batch of real images for reconstruction
			tensor_images_disc,
			features): #-- Batch of real images for discrimination
		""" Perform a validation using an input batch of images
		"""				
		##----------
		## Validate!
		with self.graph.as_default():
				...
				fetches = self.session.run(fetches_training, feed_dict=feed_dict)
				reconstructionLosses_GAN = fetches[:nb_recLosses_GAN]; nbT = nb_recLosses_GAN
				reconstructionLoss_RGB, _KLloss, _GAN_loss = fetches[nbT:]

				##-- DISPLAY INFO
# 				print(">> VAL RGB RECONSTRUCTION LOSS: ", reconstructionLoss_RGB, ">> VAL GAN RECONSTRUCTION LOSS: ", reconstructionLosses_GAN[-1], '>> VAL GAN LOSS: ', _GAN_loss)
		return reconstructionLoss_RGB, reconstructionLosses_GAN[-1],  _GAN_loss
	...
	def embedding2image(self,
		...
		return reconstructions #-- Reconstructions without the null image
# 		r_list, emb_mean, emb_logvar, dis_log, dis_rep = self.session.run([self.reconstructions_list_op,
# 															self.embedding_means, self.embedding_logvars, self.discriminator_logits, self.discriminator_representations], feed_dict = feed_dict)
# 		return r_list, emb_mean, emb_logvar, dis_log, dis_rep
