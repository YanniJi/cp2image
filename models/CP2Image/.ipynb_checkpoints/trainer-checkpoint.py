...
# 				feed_dict[self.images_ph]      = tensor_images
				feed_dict[self.images_ph]      = self._rand_img
...
# 				feed_dict[self.embedding_ph] = np.zeros([0,1,1,embeddingSize], dtype=np.float32)
				feed_dict[self.embedding_ph] = np.random.normal(size=[128,1,1,embeddingSize]).astype(np.float32)
				...
				self._file2record_loss.write('{}: RGB RECON LOSS: {:.4f}, GAN LOSS: {:.4f}\n'.format(self._step, reconstructionLoss_RGB, reconstructionLosses_GAN[-1] ))
